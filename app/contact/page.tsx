          ...
          <div className="grid lg:grid-cols-2 gap-16 lg:gap-20 items-start">
            ...
            >
              ...
              >
                ...
                <span className="text-xs font-medium text-accent-teal-light">Typically respond within 24 hours</span>
              </motion.div>
              ...
              <div className="space-y-6 mb-10">
                ...
                >
                  <div className="w-12 h-12 rounded-xl bg-surface-elevated border border-border flex items-center justify-center group-hover:border-accent-teal/50 group-hover:bg-accent-teal/5 transition-all duration-300">
                    <svg className="w-5 h-5 text-accent-teal-light" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                ...
                {/* Schedule a Call */}
                <motion.a
                  href="https://calendly.com/procedure"
                  target="_blank"
                ...
                >
                  ...
                  <div>
                    <p className="text-sm text-text-muted">Ready to talk now?</p>
                    <p className="text-text-primary font-medium group-hover:text-accent-teal-light transition-colors">
              ...
              <div className="pt-8 border-t border-border">
                <div className="grid grid-cols-2 gap-6">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-text-primary">Confidential</p>
                      <p className="text-xs text-text-muted">Your data is encrypted and secure</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <div className="w-8 h-8 rounded-lg bg-accent-teal/10 flex items-center justify-center flex-shrink-0 mt-0.5">
                    ...
                    <div>
                      <p className="text-sm font-medium text-text-primary">Fast Response</p>
                      <p className="text-xs text-text-muted">We reply within 24 hours</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <div className="w-8 h-8 rounded-lg bg-accent-teal/10 flex items-center justify-center flex-shrink-0 mt-0.5">
                    ...
                    <div>
                      <p className="text-sm font-medium text-text-primary">Expert Team</p>
                      <p className="text-xs text-text-muted">Speak directly with engineers</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <div className="w-8 h-8 rounded-lg bg-accent-teal/10 flex items-center justify-center flex-shrink-0 mt-0.5">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-text-muted">Teams across time zones</p>
                    </div>
                  </div>
                </div>
              </div>
                  ...
                  {isSubmitted ? (
                    ...
                    >
                      <div className="w-16 h-16 mx-auto mb-6 rounded-full bg-accent-teal/20 flex items-center justify-center">
                        <svg className="w-8 h-8 text-accent-teal-light" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                      ...
                      >
                        ...
                        {isSubmitting && (
                          <div className="absolute inset-0 flex items-center justify-center">
                            <svg className="w-5 h-5 animate-spin text-text-primary" fill="none" viewBox="0 0 24 24">
                      ...
                      <p className="text-xs text-text-muted text-center">
                        ...
                        <Link href="/privacy" className="text-accent-teal-light hover:underline">
                          Privacy Policy
                        </Link>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          ...
          >
            ...
            <p className="text-text-secondary max-w-2xl mx-auto">
              Our distributed teams work across time zones to deliver continuous progress on your projects.
            </p>
          </motion.div>
          ...
          >
            {[
              { city: "San Francisco", country: "United States", timezone: "PST (UTC-8)", flag: "us" },
              { city: "London", country: "United Kingdom", timezone: "GMT (UTC+0)", flag: "gb" },
              { city: "Singapore", country: "Singapore", timezone: "SGT (UTC+8)", flag: "sg" },
              { city: "Sydney", country: "Australia", timezone: "AEDT (UTC+11)", flag: "au" },
            ].map((location, idx) => (
              <motion.div
                key={location.city}
                initial={{ opacity: 0, y: 20 }}
              ...
              >
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-8 h-8 rounded-lg bg-surface border border-border flex items-center justify-center text-lg">
                    {location.flag === "us" && (
                      <span role="img" aria-label="United States">US</span>
                    )}
                    {location.flag === "gb" && (
                      <span role="img" aria-label="United Kingdom">UK</span>
                    )}
                    {location.flag === "sg" && (
                      <span role="img" aria-label="Singapore">SG</span>
                    )}
                    {location.flag === "au" && (
                      <span role="img" aria-label="Australia">AU</span>
                    )}
                  </div>
                  <div>
                    <p className="font-semibold text-text-primary">{location.city}</p>
