        ...
        <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-6 sm:gap-8">
          {values.map((card, idx) => (
            <motion.div
              ...
              className={`p-8 rounded-2xl bg-white dark:bg-zinc-800/30 border border-zinc-200 dark:border-zinc-800 ${card.hoverBorder} transition-colors shadow-sm`}
            >
              <div className={`w-12 h-12 rounded-xl ${card.iconBg} flex items-center justify-center mb-6`}>
                {card.icon}
              </div>
              <h3 className="text-xl font-semibold text-zinc-900 dark:text-white mb-3">{card.title}</h3>
