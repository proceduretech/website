    ...
    <div className={cn("w-full", className)} ref={containerRef}>
      <div ref={ref} className="relative max-w-7xl mx-auto">
        {data.map((item, index) => (
          ...
          >
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-[100px] md:w-auto shrink-0">
              <div className="h-10 absolute left-3 md:left-3 w-10 rounded-full bg-surface flex items-center justify-center">
                <div className="h-4 w-4 rounded-full bg-cta border border-cta/50 shadow-lg shadow-cta/25" />
              </div>
              <h3 className="hidden md:block text-xl md:pl-16 md:text-3xl font-bold text-text-muted">
                {item.title}
              </h3>
            </div>

            <div className="relative pl-20 pr-4 md:pl-4 w-full">
              <h3 className="md:hidden block text-xl mb-4 text-left font-bold text-text-muted">
                {item.title}
              </h3>
              {item.content}
            </div>
